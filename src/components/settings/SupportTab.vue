<script setup>
/**
 * SupportTab.vue — S10 Settings Component
 *
 * Advanced only. System information, support bundle download,
 * and admin-only setup reset.
 *
 * Extracted from SettingsView.vue System Info + Administration sections.
 *
 * Wires:
 * - GET /support/bundle.zip — support bundle download
 * - POST /setup/reset       — reset setup wizard
 */
...
const hostInfo = computed(() => {
  ...
  if (systemStore.piModel) platform = systemStore.piModel
...
async function downloadBundle() {
  ...
  try {
    ...
    bundleError.value = e.message || 'Failed to download support bundle'
  } finally {
    setTimeout(() => { bundleLoading.value = false }, 2000)
  }
}

// ─── Setup Reset (Admin Only) ────────────────────────────────
...
const hostnameValid = computed(() => hostnameInput.value && isValidHostname(hostnameInput.value))
const hostnameDirty = computed(() => hostnameInput.value !== (systemStore.hostnameValue || ''))
...
const timezoneDirty = computed(() => timezoneInput.value !== (systemStore.timezone || ''))
...
async function saveTimezone() {
  if (!timezoneInput.value) { timezoneError.value = 'Please select a timezone'; return }
...
onMounted(async () => {
  ...
  if (authStore.isAdmin) {
    setupStore.fetchStatus().then(() => { setupStatusLoaded.value = true })
  }
})
  ...
  <div class="space-y-6">
    ...
    <section class="animate-fade-in">
      ...
      <div class="p-4 rounded-xl border border-theme-primary bg-theme-card space-y-5">
        <!-- Hostname -->
        <div>
          <label for="support-hostname" class="block text-xs font-semibold text-theme-primary mb-2">Hostname</label>
          ...
          <div class="flex gap-2">
            <input
              ...
              type="text"
              class="flex-1 px-3 py-2 rounded-lg border border-theme-primary bg-theme-input text-theme-primary placeholder-theme-muted text-sm font-mono focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent transition-colors"
              placeholder="cubeos"
              maxlength="63"
        ...
        <!-- Timezone -->
        <div>
          <label for="support-timezone" class="block text-xs font-semibold text-theme-primary mb-2">Timezone</label>
          ...
          <p v-if="currentTimeInZone" class="text-xs text-theme-muted mt-1.5">Current: {{ currentTimeInZone }}</p>
          <p v-else class="text-xs text-theme-muted mt-1.5">System clock and log timestamps</p>
        </div>
      </div>
    </section>
    ...
    <section class="animate-fade-in" style="animation-delay: 75ms">
      <div class="flex items-center gap-2.5 mb-3">
        <div class="w-8 h-8 rounded-lg bg-accent-muted flex items-center justify-center">
          <Icon name="LifeBuoy" :size="16" class="text-accent" />
        </div>
        <div>
          <h2 class="text-sm font-semibold text-theme-primary">Support Bundle</h2>
          <p class="text-xs text-theme-tertiary">Download diagnostics for troubleshooting</p>
        </div>
      </div>
      ...
      <div class="p-4 rounded-xl border border-theme-primary bg-theme-card">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-medium text-theme-primary">Download Support Bundle</p>
            <p class="text-xs text-theme-muted">Contains system logs, configuration, and diagnostics</p>
          </div>
          <button
            @click="downloadBundle"
            :disabled="bundleLoading"
            aria-label="Download support bundle"
            class="px-3 py-1.5 rounded-lg btn-accent text-xs font-medium disabled:opacity-50 flex items-center gap-1.5"
          >
            <Icon :name="bundleLoading ? 'Loader2' : 'Download'" :size="14" :class="bundleLoading ? 'animate-spin' : ''" />
            {{ bundleLoading ? 'Downloading...' : 'Download .zip' }}
          </button>
        </div>
        <p v-if="bundleError" class="text-xs text-error mt-2">{{ bundleError }}</p>
      </div>
    </section>

    <!-- Administration (Admin Only) -->
    <section v-if="authStore.isAdmin" class="animate-fade-in" style="animation-delay: 100ms">
      <div class="flex items-center gap-2.5 mb-3">
      ...
      <div class="p-4 rounded-xl border border-warning/20 bg-warning-muted/30">
        <div class="flex items-center justify-between">
          <div>
            <h4 class="text-xs font-semibold text-theme-primary">Reset Setup Wizard</h4>
            <p class="text-xs text-theme-tertiary">Re-run the first-boot setup wizard</p>
          </div>
          <button
