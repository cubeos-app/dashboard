...
// ==========================================
// Ruleset Actions (Save / Restore / Reset)
// ==========================================
  ...
  <div class="p-4 sm:p-6 max-w-7xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-2xl font-bold text-theme-primary">Firewall</h1>
          <p class="text-sm text-theme-tertiary mt-0.5">Manage firewall rules and network security</p>
        </div>
      </div>
      <button
        @click="fetchAll"
    ...
    <div class="bg-theme-card rounded-xl border border-theme-primary">
      ...
      <Transition name="slide">
        <div v-if="showQuickService" class="p-4 border-b border-theme-primary bg-theme-secondary">
          ...
          <div class="mt-3 flex items-center gap-2">
            <input
              v-model="quickService.service"
              type="text"
              placeholder="Custom service name"
              class="flex-1 px-3 py-2 bg-theme-input border border-theme-primary rounded-lg text-sm text-theme-primary placeholder-theme-muted focus:outline-none focus:border-[color:var(--accent-primary)]"
        ...
        <div v-if="showAddRule" class="p-4 border-b border-theme-primary bg-theme-secondary">
          ...
          <div class="mt-3">
            ...
            <input
              ...
              class="w-full px-3 py-2 bg-theme-input border border-theme-primary rounded-lg text-sm text-theme-primary placeholder-theme-muted focus:outline-none focus:border-[color:var(--accent-primary)]"
      ...
      <div v-if="hasRules" class="hidden md:block overflow-x-auto">
        <table class="w-full text-sm">
          ...
          <tbody>
            <tr
              v-for="(rule, idx) in firewallStore.rules"
              :key="idx"
              class="border-b border-theme-primary last:border-0 hover:bg-theme-secondary transition-colors"
            >
              <td class="px-4 py-3">
                <span
                  class="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold uppercase"
                  :class="[ruleActionColor(rule.action), ruleActionBg(rule.action)]"
                >
                  {{ rule.action }}
                </span>
              </td>
      ...
      <div v-if="hasRules" class="md:hidden divide-y divide-theme-primary">
        ...
        >
          ...
          <div class="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
            <div>
              <span class="text-theme-muted">Port: </span>
              <span class="text-theme-primary font-mono">{{ rule.port || 'any' }}</span>
            </div>
            <div>
              <span class="text-theme-muted">Protocol: </span>
              <span class="text-theme-secondary uppercase">{{ rule.protocol || 'any' }}</span>
            </div>
            <div>
              <span class="text-theme-muted">From: </span>
              <span class="text-theme-secondary font-mono">{{ rule.from || 'any' }}</span>
            </div>
            <div>
              <span class="text-theme-muted">To: </span>
              <span class="text-theme-secondary font-mono">{{ rule.to || 'any' }}</span>
            </div>
          </div>
      ...
      <div v-if="!hasRules && !firewallStore.loading" class="p-8 text-center">
        <div class="flex justify-center mb-3">
          <Icon name="Shield" :size="40" class="text-theme-tertiary" />
        </div>
