<script setup>
/**
 * MonitoringTab.vue — S08 Component
 *
 * Advanced-only. Refactored from MonitoringView.vue.
 * Real-time system health: stats cards, SVG history charts,
 * alerts table, and threshold configuration.
 *
 * Wires: PUT /monitoring/thresholds (via store)
 *
...
// WebSocket subscription from App.vue
...
onMounted(async () => {
  ...
  if (wsSubscribe) wsSubscribe(WS_SUBSCRIBER_KEY, handleWsMessage)
  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-3">
        ...
        >
          <span class="w-2 h-2 rounded-full" :class="systemStore.wsConnected ? 'bg-success' : 'bg-theme-muted'" />
    ...
    >
      ...
      <button @click="refresh" class="inline-flex items-center px-4 py-2 rounded-lg bg-accent text-on-accent text-sm font-medium hover:opacity-90 transition-opacity">
    ...
    <template v-else>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div class="p-4 rounded-xl bg-theme-card border border-theme-primary">
            ...
            <div class="w-full h-1.5 rounded-full bg-theme-tertiary overflow-hidden">
              <div class="h-full rounded-full transition-all duration-500" :class="progressColor(monitoringStore.cpuPercent)" :style="{ width: progressWidth(monitoringStore.cpuPercent) }" />
            </div>
          </div>
          <div class="p-4 rounded-xl bg-theme-card border border-theme-primary">
            <div class="flex items-center gap-2 mb-3">
            ...
            <div class="text-xs text-theme-tertiary mb-2">{{ formatBytes(monitoringStore.memoryUsed) }} / {{ formatBytes(monitoringStore.memoryTotal) }}</div>
            <div class="w-full h-1.5 rounded-full bg-theme-tertiary overflow-hidden">
              <div class="h-full rounded-full transition-all duration-500" :class="progressColor(monitoringStore.memoryPercent)" :style="{ width: progressWidth(monitoringStore.memoryPercent) }" />
            </div>
          </div>
          <div class="p-4 rounded-xl bg-theme-card border border-theme-primary">
            <div class="flex items-center gap-2 mb-3">
            ...
            <div class="text-xs text-theme-tertiary mb-2">{{ formatBytes(monitoringStore.diskUsed) }} / {{ formatBytes(monitoringStore.diskTotal) }}</div>
            <div class="w-full h-1.5 rounded-full bg-theme-tertiary overflow-hidden">
              <div class="h-full rounded-full transition-all duration-500" :class="progressColor(monitoringStore.diskPercent)" :style="{ width: progressWidth(monitoringStore.diskPercent) }" />
            </div>
          </div>
          <div class="p-4 rounded-xl bg-theme-card border border-theme-primary">
            <div class="flex items-center gap-2 mb-3">
            ...
            <div class="text-2xl font-semibold mb-2" :class="tempColorClass(monitoringStore.temperature)">{{ formatTemp(monitoringStore.temperature) }}</div>
            <div class="w-full h-1.5 rounded-full bg-theme-tertiary overflow-hidden">
              <div class="h-full rounded-full transition-all duration-500" :class="monitoringStore.temperature > 60 ? 'bg-error' : monitoringStore.temperature > 50 ? 'bg-warning' : 'bg-success'" :style="{ width: progressWidth(monitoringStore.temperature ?? 0) }" />
            </div>
          </div>
        </div>
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex gap-1 p-0.5 bg-theme-tertiary rounded-lg" role="radiogroup" aria-label="History time period">
            <button
              ...
              :class="monitoringStore.historyPeriod === p.id ? 'bg-theme-card text-theme-primary shadow-sm' : 'text-theme-muted hover:text-theme-secondary'"
            >
              {{ p.label }}
            </button>
          </div>
        ...
        <div v-else class="space-y-4">
          ...
          <div class="p-4 rounded-xl bg-theme-card border border-theme-primary">
            <div class="flex items-center gap-2 mb-3">
              ...
              <span v-if="hoveredChart === 'cpu' && hoveredDataPoint" class="ml-auto text-xs text-theme-tertiary">
                {{ formatTimestamp(hoveredDataPoint.timestamp) }} — {{ Math.round(hoveredDataPoint.cpu ?? 0) }}%
              </span>
            </div>
            <svg :viewBox="`0 0 ${chartWidth} ${chartHeight}`" class="w-full h-28 sm:h-32" preserveAspectRatio="none" @mousemove="onChartMouseMove($event, 'cpu')" @mouseleave="onChartMouseLeave" @touchmove.prevent="onChartMouseMove($event, 'cpu')" @touchend="onChartMouseLeave" role="img" aria-label="CPU usage history chart">
              <line v-for="y in [0, 25, 50, 75, 100]" :key="y" x1="0" :y1="yScale(y)" :x2="chartWidth" :y2="yScale(y)" stroke="currentColor" stroke-opacity="0.1" stroke-dasharray="4,4" />
              <polygon :points="cpuFillPoints" fill="var(--accent-primary)" fill-opacity="0.1" />
              <polyline :points="cpuPoints" fill="none" stroke="var(--accent-primary)" stroke-width="2" stroke-linejoin="round" />
            </svg>
          </div>
          <!-- Memory Chart -->
          <div class="p-4 rounded-xl bg-theme-card border border-theme-primary">
            <div class="flex items-center gap-2 mb-3">
              ...
              <span v-if="hoveredChart === 'memory' && hoveredDataPoint" class="ml-auto text-xs text-theme-tertiary">
                {{ formatTimestamp(hoveredDataPoint.timestamp) }} — {{ Math.round(hoveredDataPoint.memory ?? 0) }}%
              </span>
            </div>
            <svg :viewBox="`0 0 ${chartWidth} ${chartHeight}`" class="w-full h-28 sm:h-32" preserveAspectRatio="none" @mousemove="onChartMouseMove($event, 'memory')" @mouseleave="onChartMouseLeave" @touchmove.prevent="onChartMouseMove($event, 'memory')" @touchend="onChartMouseLeave" role="img" aria-label="Memory usage history chart">
              <line v-for="y in [0, 25, 50, 75, 100]" :key="y" x1="0" :y1="yScale(y)" :x2="chartWidth" :y2="yScale(y)" stroke="currentColor" stroke-opacity="0.1" stroke-dasharray="4,4" />
              <polygon :points="memoryFillPoints" fill="var(--success)" fill-opacity="0.1" />
              <polyline :points="memoryPoints" fill="none" stroke="var(--success)" stroke-width="2" stroke-linejoin="round" />
            </svg>
          </div>
          <!-- Disk Chart -->
          <div class="p-4 rounded-xl bg-theme-card border border-theme-primary">
            <div class="flex items-center gap-2 mb-3">
              ...
              <span v-if="hoveredChart === 'disk' && hoveredDataPoint" class="ml-auto text-xs text-theme-tertiary">
                {{ formatTimestamp(hoveredDataPoint.timestamp) }} — {{ Math.round(hoveredDataPoint.disk ?? 0) }}%
              </span>
            </div>
            <svg :viewBox="`0 0 ${chartWidth} ${chartHeight}`" class="w-full h-28 sm:h-32" preserveAspectRatio="none" @mousemove="onChartMouseMove($event, 'disk')" @mouseleave="onChartMouseLeave" @touchmove.prevent="onChartMouseMove($event, 'disk')" @touchend="onChartMouseLeave" role="img" aria-label="Disk usage history chart">
              <line v-for="y in [0, 25, 50, 75, 100]" :key="y" x1="0" :y1="yScale(y)" :x2="chartWidth" :y2="yScale(y)" stroke="currentColor" stroke-opacity="0.1" stroke-dasharray="4,4" />
              <polygon :points="diskFillPoints" fill="var(--warning)" fill-opacity="0.1" />
              <polyline :points="diskPoints" fill="none" stroke="var(--warning)" stroke-width="2" stroke-linejoin="round" />
            </svg>
          </div>
        </div>
      </template>
      ...
      <template v-if="activeTab === 'alerts'">
        <div v-if="!monitoringStore.hasAlerts" class="p-12 rounded-xl bg-theme-card border border-theme-primary text-center">
      ...
      <template v-if="activeTab === 'settings'">
        <div class="space-y-6">
          <div class="p-5 rounded-xl bg-theme-card border border-theme-primary">
            ...
            <p class="text-xs text-theme-tertiary mb-4">Configure the percentage thresholds at which alerts are triggered.</p>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
              <div>
                <label for="threshold-cpu" class="flex items-center gap-1 text-sm font-medium text-theme-secondary mb-1">
                  <Icon name="Cpu" :size="14" class="text-accent" /> CPU Alert (%)
                </label>
                <input id="threshold-cpu" v-model.number="thresholdForm.cpu_percent" type="number" min="0" max="100" class="w-full px-3 py-2 rounded-lg border border-theme-primary bg-theme-input text-theme-primary focus:outline-none focus:border-accent text-sm" />
              </div>
              <div>
                <label for="threshold-memory" class="flex items-center gap-1 text-sm font-medium text-theme-secondary mb-1">
                  <Icon name="Server" :size="14" class="text-accent" /> Memory Alert (%)
                </label>
                <input id="threshold-memory" v-model.number="thresholdForm.memory_percent" type="number" min="0" max="100" class="w-full px-3 py-2 rounded-lg border border-theme-primary bg-theme-input text-theme-primary focus:outline-none focus:border-accent text-sm" />
              </div>
              <div>
                <label for="threshold-disk" class="flex items-center gap-1 text-sm font-medium text-theme-secondary mb-1">
                  <Icon name="HardDrive" :size="14" class="text-accent" /> Disk Alert (%)
                </label>
                <input id="threshold-disk" v-model.number="thresholdForm.disk_percent" type="number" min="0" max="100" class="w-full px-3 py-2 rounded-lg border border-theme-primary bg-theme-input text-theme-primary focus:outline-none focus:border-accent text-sm" />
              </div>
              <div>
                <label for="threshold-temp" class="flex items-center gap-1 text-sm font-medium text-theme-secondary mb-1">
                  <Icon name="Thermometer" :size="14" class="text-accent" /> Temperature Alert (°C)
                </label>
                <input id="threshold-temp" v-model.number="thresholdForm.temperature_c" type="number" min="0" max="100" class="w-full px-3 py-2 rounded-lg border border-theme-primary bg-theme-input text-theme-primary focus:outline-none focus:border-accent text-sm" />
              </div>
            </div>
            <div class="flex items-center gap-3 flex-wrap">
              <button @click="saveThresholds" :disabled="monitoringStore.savingThresholds" class="inline-flex items-center px-4 py-2 rounded-lg bg-accent text-on-accent text-sm font-medium hover:opacity-90 disabled:opacity-50 transition-opacity" aria-label="Save alert thresholds">
                <Icon v-if="monitoringStore.savingThresholds" name="Loader2" :size="16" class="animate-spin mr-2" />
                Save Thresholds
              </button>
              <span v-if="saveSuccess" class="inline-flex items-center gap-1 text-xs text-success">
                <Icon name="CheckCircle" :size="14" /> Saved successfully
              </span>
              <span v-if="saveError" class="inline-flex items-center gap-1 text-xs text-error">
                <Icon name="XCircle" :size="14" /> {{ saveError }}
              </span>
            </div>
          </div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="flex items-center justify-between p-3 rounded-lg bg-theme-secondary">
                  ...
                  <div class="flex items-center gap-1.5">
                    <span class="w-2 h-2 rounded-full" :class="systemStore.wsConnected ? 'bg-success' : 'bg-theme-muted'" />
                    <span class="text-sm font-medium" :class="systemStore.wsConnected ? 'text-success' : 'text-theme-muted'">
