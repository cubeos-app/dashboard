<script setup>
/**
 * ProfilesTab.vue — S05 Refactor (from appmanager/ProfilesTab.vue)
 *
 * Advanced-only tab for operational profile management.
 * Refactored to use consolidated profiles store (setProfileApp moved from appmanager.js).
 * Uses apps store for app list in configure modal.
 */
...
async function createProfile() {
  creating.value = true
  try {
    await profilesStore.createProfile(newProfile.value.name, newProfile.value.name, newProfile.value.description)
...
async function activateProfile(profile) {
  ...
  try { await profilesStore.applyProfile(profile.name) } catch (e) { error.value = e.message || 'Failed to activate profile' }
...
async function deleteProfile(profile) {
  ...
  try { await profilesStore.deleteProfile(profile.name) } catch (e) { error.value = e.message || 'Failed to delete profile' }
}

async function openConfigureModal(profile) {
  selectedProfile.value = profile
  const details = await profilesStore.getProfile(profile.name)
  if (details) {
    profileApps.value = appsStore.apps.map(app => ({
      ...app,
...
async function toggleAppInProfile(app) {
  ...
  try {
    await profilesStore.setProfileApp(selectedProfile.value.name, app.id, newEnabled)
...
<template>
  <div>
    ...
    <!-- Empty State -->
    <div v-if="profilesStore.profiles.length === 0" class="text-center py-12">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <div v-for="profile in profilesStore.profiles" :key="profile.id" :class="['bg-theme-secondary rounded-lg border p-4 transition-colors', profile.is_active ? 'border-success bg-success-muted/20' : 'border-theme-primary hover:border-accent/50']">
